...
# First-line control marker that lets an agent declare a semantic cron failure even when its
# own turn completed normally (for example, after a delegated child failed).
CRON_FAILURE_MARKER = "[CRON_FAILURE]"


def _cron_failure_marker_error(text: str) -> Optional[str]:
    """Return the declared failure reason from an exact standalone first-line marker.

    Mid-report mentions and quoted/indented markers remain ordinary output. The caller keeps
    the original response intact for the saved run document.
    """
