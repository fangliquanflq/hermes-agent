...
def _build_gateway_argv(*, profile: str | None = None, home: Path | None = None) -> tuple[list[str], str, dict[str, str]]:
...
def _spawn_detached(
    script_path: Path | None = None, *, profile: str | None = None, home: Path | None = None,
) -> int:
    ...
    """
    _assert_windows()
    argv, working_dir, env_overlay = _build_gateway_argv(profile=profile, home=home)
    ...
    log_dir = (Path(home) if home is not None else _hermes_home()) / "logs"
...
def _gateway_pids_for_home(home: Path | None, *, all_profiles: bool) -> list[int]:
    if home is None:
...
def _confirm_gateway_stable(
    ...
    while time.monotonic() < confirm_deadline:
        ...
        pids = _gateway_pids_for_home(home, all_profiles=all_profiles)
        if not pids:
            return []
    return pids
    ...
    while time.monotonic() < deadline:
        ...
        if pids:
            confirmed = _confirm_gateway_stable(
                pids, confirm_s, interval_s, all_profiles=all_profiles, home=home
            )
            if confirmed:
                return confirmed
...
def _start_attestation_path(home: Path | None = None) -> Path:
    return (Path(home) if home is not None else _hermes_home()).joinpath(*_START_ATTESTATION_RELATIVE)
...
def _attested_pid_exited_cleanly(
    pid: int, create_time: float | None = None, home: Path | None = None,
) -> bool:
    ...
    try:
        ...
        ledger_home = Path(home) if home is not None else _hermes_home()
        data = json.loads(get_lifecycle_sentinel_path(ledger_home).read_text(encoding="utf-8"))
    except OSError:
        return False
    except Exception:
...
def attested_death_generation(current_pids: list[int], home: Path | None = None) -> str | None:
    ...
    if not attested or not _attestation_within_horizon(data) or not _attested_dead(
        attested, current_pids, data, home
    ):
        return None
