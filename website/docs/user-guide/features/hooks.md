...
- Any Python or shell hook whose payload includes an active `turn_id` may return `{"action": "halt_turn", "response": "..."}`. Hermes stops the current turn at the next safe boundary and uses `response` as the authoritative final assistant response. The first valid directive wins; `response` must be a non-empty string. Streaming hooks stay off the token path: their worker records the directive, and the stream owner cancels the provider stream when it observes that request on a later delta or completion boundary.
...
| `post_tool_call` | Observer/control | After blocked, error, or successful result; ordinary returns are ignored, while `halt_turn` ends the turn. | `tool_name`, `args`, `result`, `task_id`, `session_id`, `tool_call_id`, `turn_id`, `api_request_id`, `duration_ms`, `status`, `error_type`, `error_message`, `middleware_trace` | Result/error text may contain arbitrary tool or user content and secrets. |
...
| `pre_llm_call` | Directive/control | Once per turn before the loop; all valid string/`{"context": ...}` returns are joined and injected into the user message, or `halt_turn` ends the turn before the provider call. | `session_id`, `task_id`, `turn_id`, `user_message`, `conversation_history`, `is_first_turn`, `model`, `platform`, `parent_session_id`, `sender_id` | Full user message and conversation history. |
...
| `on_stream_delta` | Observer/control | Dispatched per normalized streaming text delta via the bounded observer queue; a stalled callback drops only its own oldest events. An exact `halt_turn` return requests stream cancellation and a controlled final response. | `delta`, `kind` (`text` or `reasoning`), `turn_id`, `iteration`, `session_id`, `model`, `provider`, `surface` | Delta text is raw model output; reasoning deltas require the `plugins.stream_reasoning_deltas` opt-in. |
