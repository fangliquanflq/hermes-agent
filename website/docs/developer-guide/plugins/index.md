...
| [`post_tool_call`](/user-guide/features/hooks#post_tool_call) | After any tool returns | `tool_name: str, args: dict, result: str, task_id: str, duration_ms: int` | optional `halt_turn` directive |
| [`pre_llm_call`](/user-guide/features/hooks#pre_llm_call) | Once per turn, before the tool-calling loop | `session_id: str, user_message: str, conversation_history: list, is_first_turn: bool, model: str, platform: str` | [context injection](#pre_llm_call-context-injection) or `halt_turn` |
...
Most hooks are fire-and-forget observers — their ordinary return values are ignored. The exceptions are `pre_llm_call`, which can inject context into the conversation, and `pre_tool_call`, which can return a block/approve directive. In addition, any hook carrying an active `turn_id` may return `{"action": "halt_turn", "response": "..."}` to end that turn with a controlled final response; the first valid directive wins.
